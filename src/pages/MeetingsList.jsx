...
import {BookKey, CheckCircleIcon, XCircleIcon, ClockIcon, ArrowLeftIcon, ChevronDownIcon} from 'lucide-react';
                ...
                <div className="bg-blue-50 p-4 sm:p-6 rounded-xl mb-4 sm:mb-6 border border-blue-100">
                    <div className="flex items-center justify-between mb-4 sm:mb-6">
                        <div className="flex items-center">
                            <Link to="/profile" className="mr-2 sm:mr-3 p-1.5 sm:p-2 hover:bg-blue-100 rounded-full transition-colors">
                                <ArrowLeftIcon size={18} className="text-gray-700 sm:hidden" />
                                <ArrowLeftIcon size={20} className="text-gray-700 hidden sm:block" />
                            </Link>
                            <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">Мои бронирования</h1>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                        <div className="sm:hidden border-b">
                            ...
                            >
                                ...
                                <ChevronDownIcon
                                    size={18}
                                    className={`text-gray-500 transition-transform ${mobileDropdownOpen ? 'transform rotate-180' : ''}`}
                                />
                            </button>

                            <div className={`bg-gray-50 overflow-hidden transition-all duration-500 ease-in-out ${mobileDropdownOpen ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
                                {tabs.map((tab) => {
                                    const Icon = tab.icon;
                        ...
                        <div className={`transition-all duration-500 ease-in-out p-3 sm:p-6 ${contentVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-4'}`}>
