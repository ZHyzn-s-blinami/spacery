...
const UserControl = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const response = await axios.get('https://prod-team-5-qnkvbg7c.final.prodcontest.ru/api/user/all', {
                headers: {
            ...
            setError(err.response?.data?.message || err.message || 'Ошибка при получении списка пользователей');
            setLoading(false);
        }
    };
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            setUpdateError(err.response?.data?.message || err.message || 'Ошибка при обновлении пользователя');
        }
    };
    ...
    const handleCreateSubmit = async (e) => {
        ...
        try {
            ...
            setCreateError(err.response?.data?.message || err.message || 'Ошибка при создании пользователя');
        }
    };
    ...
    const handleDeleteSubmit = async () => {
        try {
            ...
            setDeleteError(err.response?.data?.message || err.message || 'Ошибка при удалении пользователя');
        }
    };
        ...
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                    <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">Управление пользователями</h1>
                                ...
                                <table ref={tableRef} className="min-w-full bg-white table-auto">
                                    ...
                                    {currentUsers.map((user) => (
                                        <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                            <td className="py-3 px-3 text-sm whitespace-nowrap overflow-hidden text-ellipsis">{user.id.substring(0, 8)}...</td>
                                            <td className="py-3 px-3 text-sm font-medium whitespace-nowrap overflow-hidden text-ellipsis">{user.email}</td>
                                            <td className="py-3 px-3 text-sm whitespace-nowrap overflow-hidden text-ellipsis">{user.name || '-'}</td>
                                            ...
                                            <td className="py-3 px-3 text-sm max-w-[150px] truncate">{user.description || '-'}</td>
                                            <td className="py-3 px-3">
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.verified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                                    {user.verified ? 'Verified' : 'Unverified'}
                                                </span>
                                            </td>
                                            <td className="py-3 px-3 text-sm whitespace-nowrap">{formatDate(user.createdAt)}</td>
                                            <td className="py-3 px-3">
                                ...
                                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                    <div>
                                        <p className="text-sm text-gray-700">
                                            Показаны записи с <span className="font-medium">{indexOfFirstUser + 1}</span> по <span className="font-medium">{Math.min(indexOfLastUser, users.length)}</span> из <span className="font-medium">{users.length}</span> записей
                                        </p>
                                    </div>
                                    <div>
